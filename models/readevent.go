...
func QueryReadEvent(req *ReadEventRequest, response *Response) {
	...
	go func() {
		if startTime, ok := utils.TimeStringToString(req.TimeRange[0]); nil == ok {
			if endTime, ok := utils.TimeStringToString(req.TimeRange[1]); nil == ok {
				mmsql := "SELECT gidNum, usrNum, chgNum, freNum, tfNum, sumNum, timeStamp FROM read_event WHERE timeStamp >= " +
